...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate V5 offline with coarse-only retrieval.")
    parser.add_argument("--config", default="_V5/configs/default.yaml")
    parser.add_argument("--query-in", default="data/V5/processed/query.jsonl")
    ...
    if top_ks:
        ...
        parsed_top_ks = tuple(int(item) for item in cfg_get(cfg, "evaluation.top_k_metrics", [1, 3, 5, 10]))
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    eval_cfg = EvalConfig(
        config_path=args.config,
        query_path=resolve_path(args.query_in),
        run_dir=resolve_path(Path("_V5") / "runs" / "eval_offline" / timestamp),
        split=args.split,
    ...
    print(f"[v5] coarse eval split={args.split} queries={result['query_count']} -> {eval_cfg.run_dir}")
    print(
        f"[v5] coarse RetrievalRecall@N={result['coarse']['retrieval_recall_at_n']:.4f} "
        f"MRR={result['coarse']['mrr']:.4f} Top1={result['coarse']['top1']:.4f}"
    )
