...
from agentmemory_v3.query_utils import resolve_query_text
...
def build_e5_cache(config_path: str | Path, *, alias: str = "", limit_memory: int = 0, limit_query: int = 0) -> dict:
    ...
    memory_texts = [str(row.get("raw_text") or "") for row in memory_rows]
    ...
    manifest = {
        ...
        "fields": ["memory_coarse", "query_coarse"],
    }
    write_cache_bundle(
        paths,
        manifest=manifest,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build V5 E5 vector cache.")
    ...
    print(
        f"[v5] e5 cache alias={result['alias']} memory={result['memory_count']} "
