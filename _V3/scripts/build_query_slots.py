        ...
        if max(1, int(workers)) == 1:
            for idx, row in enumerate(query_rows, start=1):
                ...
                if idx % 20 == 0 or idx == len(query_rows):
                    print(f"[v3][query_slots] scanned={idx}/{len(query_rows)} newly_written={processed}")
        else:
            with concurrent.futures.ThreadPoolExecutor(max_workers=max(1, int(workers))) as executor:
                ...
                for idx, future in enumerate(concurrent.futures.as_completed(future_map), start=1):
                    ...
                    if idx % 20 == 0 or idx == len(pending_rows):
                        print(f"[v3][query_slots] completed={idx}/{len(pending_rows)} newly_written={processed}")
        for item in buffered:
            f.write(json.dumps(item, ensure_ascii=False) + "\n")
        f.flush()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build V3 query slots via DeepSeek IE.")
    ...
    parser.add_argument("--out", default="data/V3/processed/query_slots.jsonl")
    ...
    print(f"[v3] query slots count={result['count']} written_now={result['written_now']} -> {result['out']}")
