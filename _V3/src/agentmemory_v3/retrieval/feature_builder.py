...
def build_query_context(dense_index: Any, query_text: str, slot_row: dict | None = None) -> dict[str, Any]:
    q_text = (query_text or "").strip()
    q_vec = dense_index.transform_texts([q_text])[0] if q_text else np.zeros(dense_index.matrix.shape[1], dtype=np.float32)
    ...
    query_slot_matrix = dense_index.transform_texts(query_slot_texts)
    query_slot_vectors = {field: query_slot_matrix[idx] for idx, field in enumerate(SLOT_FIELDS)}
    return {
        "query_text": q_text,
        "q_vec": q_vec,
