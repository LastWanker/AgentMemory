...
def train_bipartite_model(config: TrainConfig) -> dict[str, Any]:
    ...
    query_seq_map = load_query_slot_sequences(config.query_slots_path, dense_index, query_path=config.query_path)
    train_loader = DataLoader(
...
def _fit_feature_model(model, train_loader, valid_loader, optimizer, device, epochs):
    ...
    for epoch in range(1, int(epochs) + 1):
        ...
        print(
            f"[v3][train] epoch={epoch} train_loss={train_metrics['loss']:.4f} train_mrr={train_metrics['mrr']:.4f} "
