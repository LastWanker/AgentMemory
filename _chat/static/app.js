...
let currentRetrievalLabel = "reranker";
...
async function createSession() {
  ...
  currentHistory = [];
  updateRetrievalLabel(document.getElementById("coarse-only-toggle").checked ? "coarse-only" : "reranker");
  updateSessionLabel();
...
function updateRetrievalLabel(label) {
  currentRetrievalLabel = label || "reranker";
...
async function sendMessage(text) {
  const coarseOnly = document.getElementById("coarse-only-toggle").checked;
  const retrievalLabel = coarseOnly ? "coarse-only" : "reranker";
  ...
  const resp = await fetch("/api/chat", {
    ...
    body: JSON.stringify({ session_id: currentSessionId, text, coarse_only: coarseOnly }),
  });
  const data = await resp.json();
  if (pendingAssistantTimer) {
    clearTimeout(pendingAssistantTimer);
