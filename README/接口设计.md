...
这是库里的“条”。不管后面怎么扩展，它都不变。

- 建议存成 JSONL（每行一个 JSON），或者 SQLite 表。

**必备字段：**

- `mem_id`：字符串 ID（uuid 或递增）
- `text`：原文（必须有）
- `created_at`：时间戳（可选）
- `source`：来源（chat/log/web/manual）
- `thread_id`：对话线程/主题 ID（可选）
...
### 1.4 RouteOutput（路由输出）

同一份检索在不同阶段的“路由输出”不一样，接口需要能表达软/半硬/硬路由。

**字段：**

- `policy`：`soft` / `half_hard` / `hard`
- `weights`：`{mem_id: weight}`（软路由输出）
- `selected_ids`：主干可见记忆（半硬/硬路由）
- `metrics`：路由熵、Top-k 权重占比、一致性标记
- `explain`：可解释信息（权重分布、Top-k、选中理由）

---

## 2) 组件接口（模块边界清晰，MVP 也能跑）

我按下面的“接口”写代码，先实现最小版本，后面只加实现类，不改调用方。
...
- `cluster_centers_r`：token 向量聚类取 r 个中心（想要的 3~8 个“场方向”）
- `hybrid`：“1 个粗句向量 + 32 个 token 向量组”（粗召回+精排两用）

**MVP 推荐我直接用：**

- 记忆：`token_pool_top32`（32 个向量就很够用）
...
**MVP 现实做法（适合 2060 + 十几小时预算）：**

- 粗召回用单向量 ANN：FAISS（CPU/GPU 都行）
...
我现在先把 Trainer 留空也行，因为 MVP 不靠它也能检索。

**训练策略补充（课程学习）**  
- 从软路由开始：N 全可见，权重平滑。  
- 逐步缩小可见集合：N → k，同时逐步加大错误路由惩罚。  

### 2.6 Router（路由器，阶段化接入）

**职责**：将候选集合映射为“可见集合 + 权重”。

**接口建议：**

- `route(candidate_scores, policy, top_k) -> RouteOutput`

**阶段定义：**

- **Phase 1 / 软路由**：返回 `weights`，不裁剪计算路径。  
- **Phase 2 / 半硬路由**：返回 `selected_ids + weights`，主干只看 top-k。  
- **Phase 3 / 硬路由**：只返回 `selected_ids`，未选中记忆不可见。  
...
```
data/
  memory_normal.jsonl       # MemoryItem 原始库（默认）
  memory_easy.jsonl         # 小评测集/对照版本
  pairs.jsonl               # (x,y) 训练对（可选）
  eval_normal.jsonl         # 小评测集（默认）
  eval_easy.jsonl           # 小评测集/对照版本

embeddings/
  bert_tokenpool32/
    meta.json               # encoder_id, strategy, dims, n_vecs...
    vectors.f16             # 二进制向量（或 npy/mmap）
    aux.jsonl               # token 位置/关键词等可解释信息（可选）
  coarse_sentence/
    faiss.index             # 粗召回索引
...
到这一步，我已经能做到：输入一句话 → 找出语义相近的记忆（虽然不够“联想”，但能跑）。
...
这就是“无训练 MVP”：用预训练模型 + 向量组匹配，已经比“单向量 RAG”灵活很多。
...
- top-k 记忆
- 分数
- 哪几个 query token 与记忆 token 对齐贡献最大（靠这个调策略）
- 路由解释：权重分布、Top-k 权重占比、路由熵
- 反事实调试位：删掉最高权重记忆再跑一次（开发者模式）

---

## 5) “要跑十几个小时”应该花在哪里（合理分配）

- 建库向量化（一次性离线）：几小时到十几小时，完全合理
- 检索在线：粗召回毫秒级；精排 top-1000 * (32x32 余弦) 也能做到可接受（尤其可以先 top-300 精排）
...
- 要让“索引场更像场”：把 Query 的 Vectorizer 从 `token_pool` 换成 `cluster_centers_6`
- 要“更懂关系而非相似”：加一个 `RerankerScorer`，只对 top-200 跑
- 要“自监督提升”：Trainer 训练一个小 `projection_head`，然后 Encoder 输出后过一层 head（接口不变）
- 要“个性化”：把 `projection_head` 做成用户专属文件即可（每人几 MB）
