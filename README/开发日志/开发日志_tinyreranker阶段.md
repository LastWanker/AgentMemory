...
## 2026-02-23（followup + supplemental 正式接入）

### 1) non-preview supplemental 生成入口落地
- 新增一键脚本：`scripts/memory_processer/runtime/build_chat_supplemental_eval.py`
- 默认 non-preview 输出：`data/Processed/eval_chat_supplemental.api.jsonl`
- preview 保留：`data/Processed/eval_chat_supplemental.api_preview.jsonl`
- 该 runtime 会明确打印 preview / non-preview 的路径与条数，并可选自动重建：
  - `data/Processed/eval_chat.jsonl`
  - `data/Processed/eval_chat_followup.jsonl`

### 2) supplemental + followup 合并训练链路
- 新增数据合并脚本：`scripts/memory_indexer/build_merged_dataset.py`
  - 默认合并源：
    - `memory_followup.jsonl` + `memory_chat.jsonl`
    - `eval_followup.jsonl` + `eval_chat_supplemental.api.jsonl`
  - 默认输出：
    - `data/Processed/memory_followup_plus_chat.jsonl`
    - `data/Processed/eval_followup_plus_chat.jsonl`
  - 同步分组落盘：
    - `data/Processed/groups/followup_plus_chat/memory.jsonl`
    - `data/Processed/groups/followup_plus_chat/eval.jsonl`

- 新增一键训练评测脚本：`scripts/memory_indexer/runtime/train_eval_followup_plus_chat.py`
  - 默认 dataset：`followup_plus_chat`
  - 默认 epochs：`15`
  - 默认 bootstrap：`0`
  - 默认限流：`max_queries=2000`（`0` 可改为全量）
  - 默认评测会打印当前 learned scorer 权重路径与 meta

### 3) 权重命名统一与路径修正
- `eval_router` 默认 reranker 路径改为：`data/ModelWeights/pairwise_reranker.pt`
- `runtime/eval_hf_normal.py` 默认路径同步改为 `pairwise_reranker.pt`
- `demo.py` 默认路径同步改为 `pairwise_reranker.pt`
- `train_pairwise_reranker.py` / `train_listwise_reranker.py` 覆盖前备份命名改为：
  - `*.old_YYYYmmdd_HHMMSS.pt`
- `train_reranker.py` 默认保存名按 `loss_type` 自动切换：
  - pairwise -> `pairwise_reranker.pt`
  - listwise -> `listwise_reranker.pt`

### 4) 性能默认值修正（低算力友好）
- `configs/default.yaml`：`train_reranker.epochs` 默认由 `10` 调整为 `15`
- `configs/chat_memory.yaml`：LLM supplemental 默认并发由 `32` 下调为 `8`
- `run_ablation_matrix.py` 默认 bootstrap 由 `500` 调整为 `0`
- `runtime/ablate_followup_listwise_vs_pairwise.py` 固定 bootstrap 从 `200` 改为 `0`
- `runtime/compare_followup_pairwise_vs_listwise.py` 默认 epochs 改为 `15`
- `runtime/train_pairwise_and_listwise_reranker.py` formal profile epochs 改为 `15`

### 5) 本次实跑记录
- supplemental non-preview 实跑：
  - `data/Processed/eval_chat_supplemental.api.jsonl` = `4506` rows
  - `data/Processed/eval_chat_supplemental.api_preview.jsonl` = `18` rows（保留）

- 合并数据实跑：
  - `data/Processed/memory_followup_plus_chat.jsonl` = `3728` rows
  - `data/Processed/eval_followup_plus_chat.jsonl` = `4806` rows
  - `duplicate_mem_ids=0, renamed_query_ids=0, missing_positive_rows=0`

- pairwise 训练实跑（HF, epochs=15, max_queries=2000）：
  - 新权重：`data/ModelWeights/pairwise_reranker.pt`
  - 旧权重备份：`data/ModelWeights/pairwise_reranker.old_20260223_192404.pt`
  - eval 已打印权重 meta：`loss_type=pairwise, neg_strategy=ranked, epochs=15, seed=11, dataset=followup_plus_chat`

## 2026-02-23（评测语义修正落地：P/C/R + Recall@P + candidate_source）

### 本次变更摘要
- `scripts/memory_indexer/eval_router.py`
  - 统一评测术语落地：在 query 读取与缓存中显式保留 `positives(P)` 与 `candidates(C)`。
  - 新增候选来源开关：`--candidate-source global|eval|hybrid`。
    - `global`：仅使用检索器全局召回候选；
    - `eval`：优先使用 eval 文件自带 `candidates`；
    - `hybrid`：全局召回与 eval candidates 并集。
  - 新增评测 query 子采样参数：
    - `--max-eval-queries`（默认 1000）
    - `--eval-sample-seed`（默认 11）
    - `--eval-sample-mode random|head`（默认 random）
  - 默认一致性改为关闭（轻量默认）；显式 `--consistency-pass` 才开启双跑。
  - 指标新增并并行输出：
    - `RetrievalRecall@N`（候选池覆盖）
    - `Recall@k`（固定预算）
    - `Recall@P`（预算=正例规模，按 `min(|P|, |C|)`）
  - ablation 组名更新：
    - `baseline(auto)` 更名为 `mix(auto)`；
    - 新增 `C-only`（coarse-only 权重组）；
    - 保留旧名 alias（`baseline(auto)`/`baseline`）兼容旧命令。
  - query cache 签名版本升到 `cache_v=v3`（避免旧缓存结构污染）。

- `src/memory_indexer/retriever.py`
  - `retrieve()` 新增可选参数：
    - `candidate_ids`
    - `candidate_merge=replace|union`
    - `return_all`
  - 候选构建支持“外部 candidates 注入 + replace/union 合并”。
  - route metrics 增加：
    - `provided_candidate_size`
    - `provided_candidate_used`
    - explain 字段补充 `candidate_merge`。

- 默认配置与 runtime 脚本同步减重：
  - `configs/default.yaml`
    - `common.top_n` 默认改为 `30`
    - `eval_router.policies` 默认改为 `half_hard`
    - 新增 `eval_router.{candidate_source,max_eval_queries,eval_sample_seed,eval_sample_mode}`
    - `ablation_matrix.policies` 默认改为 `half_hard`
  - `scripts/memory_indexer/runtime/eval_hf_normal.py`
    - 默认改为 `policies=half_hard`、`ablation-groups=mix(auto)`。
  - `scripts/memory_indexer/runtime/train_eval_followup_plus_chat.py`
    - 默认 `policies=half_hard`、`ablation-groups=mix(auto)`；
    - 仅在显式 `--consistency-pass` 时开启一致性双跑。
  - `scripts/memory_indexer/runtime/compare_followup_pairwise_vs_listwise.py`
    - 评测组更新为 `mix(auto)+half_hard`，并兼容读取旧 `baseline(auto)` 记录。
  - `scripts/memory_indexer/run_ablation_matrix.py`
    - 解析逻辑兼容 `mix(auto)` 与新 eval 输出格式。

### 本地最小验证
- 语法检查：
  - `python -m py_compile scripts/memory_indexer/eval_router.py src/memory_indexer/retriever.py scripts/memory_indexer/runtime/eval_hf_normal.py scripts/memory_indexer/runtime/train_eval_followup_plus_chat.py scripts/memory_indexer/run_ablation_matrix.py scripts/memory_indexer/runtime/compare_followup_pairwise_vs_listwise.py`
- 评测冒烟（simple）：
  - `--candidate-source global`：成功输出 `RetrievalRecall@N/Recall@k/Recall@P` 三指标。
  - `--candidate-source eval`（followup 样本）：`provided=30.0/30.0`，说明 eval 自带 candidates 已参与评测。
  - `--candidate-source hybrid`：候选规模变大且指标可用，说明并集合并路径正常。

## 2026-02-23（纠偏：候选池仅粗召回 top_n，彻底停用 query 内 candidates）

### 背景
- 在讨论中确认：`candidates` 与 `positives` 被历史流程混入同一语义，导致部分样本里出现 `candidates == positives`。
- 这会直接造成“无负例”或评测语义漂移，不符合当前开发目标。

### 本次纠偏改动
- 评测侧（`scripts/memory_indexer/eval_router.py`）
  - 停用 query 内 `candidates`：只读取 `query_text + positives`。
  - 候选池统一定义为粗召回 `top_n`（句向量），并在入口强制 `candidate_mode=coarse`。
  - 三指标保留：
    - `RetrievalRecall@N = |C∩P|/|P|`
    - `Recall@k = |Top_k∩P|/|P|`
    - `Recall@P = |Top_|P|∩P|/|P|`
  - `Recall@P` 计算使用 `Top_|P|` 预算（通过检索 `top_k=max(k,|P|)` 获取）。

- 训练侧（`scripts/memory_indexer/train_reranker.py`）
  - 停用样本内 `candidates/hard_negatives` 输入。
  - 候选池固定为粗召回 `top_n`。
  - 若候选内无负例，自动从全库（排除正例）随机补负例，避免样本被静默丢弃。

- 生成侧（`src/chat_memory_processor/querygen.py`、`src/chat_memory_processor/llm_querygen.py`）
  - 不再在 query 行写出 `candidates/hard_negatives` 字段。
  - followup 输出保留核心字段：`query_id/query_text/positives/meta`。

- 配置与 runtime
  - `configs/default.yaml`：`common.candidate_mode=coarse`。
  - runtime 入口默认 `candidate-mode` 同步改为 `coarse`。

- 存量数据清理
  - 已对 `data/Processed/**/eval*.jsonl` 批量移除 `candidates/hard_negatives` 字段，避免继续误用旧产物。

### 最小验证
- `python -m py_compile ...`（覆盖 eval/train/retriever/chat query 相关脚本）通过。
- `eval_router`（simple）可跑通并输出三指标，且候选模式显示为 `coarse`。
- `train_reranker --fast-dev-run` 可跑通，候选模式会自动矫正为 `coarse`。

## 2026-02-23（补充纠偏：Recall@P 公式口径统一）
### 口径补充
- 为避免和历史文字混淆，`Recall@P` 统一记作：
  - `Recall@P = |Top_|P|∩P| / |P|`
- 解释保持不变：当粗召回候选池未覆盖全部正例（`|C∩P| < |P|`）时，`Recall@P` 实际上限仍受召回覆盖限制。
- 先前日志中出现的 `Top_{min(|P|, |C|)}` 表述视为历史口径，不再作为当前实现定义。

## 2026-02-24（用户缓存模式固化 + followup_plus_chat quick/pipeline 对齐）
### 本次变更摘要
- `scripts/memory_indexer/runtime/eval_followup_plus_chat_quick.py`
  - 修复参数访问错误：`args.hf-online` -> `args.hf_online`。
  - 默认启用用户缓存模式：`--cache-alias users --no-cache-signature`。
  - `simple` 后端自动切换到 `users_simple`，避免覆盖 HF 用户缓存。
  - 默认采样模式改为 `head`，便于稳定复现。

- `scripts/memory_indexer/eval_router.py`
  - 新增固定缓存别名参数：`--cache-alias`。
  - 新增关闭签名校验参数：`--no-cache-signature`。
  - 固定缓存名支持：`eval_cache_<alias>.jsonl` / `memory_cache_<alias>.jsonl`。
  - 保留 legacy memory cache 复用与迁移逻辑，避免重复向量化。

- runtime 入口统一为用户缓存模式（默认）：
  - `scripts/memory_indexer/runtime/train_eval_followup_plus_chat.py`
  - `scripts/memory_indexer/runtime/eval_hf_normal.py`
  - `scripts/memory_indexer/runtime/compare_followup_pairwise_vs_listwise.py`
  - 默认走 `cache_alias=users`（simple 时自动 `users_simple`），默认关闭签名校验（可用 `--use-cache-signature` 打开）。

- 训练数据合并链路补齐（identity + supplemental）：
  - `scripts/memory_indexer/build_merged_dataset.py` 支持第三路 eval 输入 `--eval-c`。
  - 默认合并：
    - `eval_followup.jsonl`
    - `eval_chat_followup.jsonl`（chat identity）
    - `eval_chat_supplemental.api.jsonl`（LLM supplemental）
  - `scripts/memory_indexer/runtime/train_eval_followup_plus_chat.py` 已默认传入 `--eval-b eval_chat_followup.jsonl` + `--eval-c <supplemental>`。

### 验证记录
- 语法检查通过：
  - `python -m py_compile scripts/memory_indexer/runtime/train_eval_followup_plus_chat.py scripts/memory_indexer/runtime/eval_hf_normal.py scripts/memory_indexer/runtime/compare_followup_pairwise_vs_listwise.py scripts/memory_indexer/runtime/eval_followup_plus_chat_quick.py scripts/memory_indexer/eval_router.py scripts/memory_indexer/build_merged_dataset.py`
- quick 评测冒烟（HF，本地缓存）：
  - `python scripts/memory_indexer/runtime/eval_followup_plus_chat_quick.py --max-eval-queries 5 --eval-sample-mode head`
  - 成功复用：
    - `data/VectorCache/memory_cache_users.jsonl`
    - `data/VectorCache/eval_cache_users.jsonl`
  - 成功输出：`runs/rt/followup_plus_chat_pipeline/<timestamp>/pairwise/eval/eval.log.txt`

## 2026-02-24（用户缓存补丁：跨数据集 query 误复用修复）
### 问题现象
- 在 `--cache-alias users --no-cache-signature` 模式下，`eval_hf_normal` 可能复用到 `followup_plus_chat` 的 `eval_cache_users.jsonl`。
- 日志仍显示“签名匹配”，与无签名模式语义不一致，易误判。

### 修复内容
- `scripts/memory_indexer/eval_router.py`
  - 新增 `count_query_rows_in_cache()`。
  - `query_cache_compatible()` 新增 `expected_query_rows` 参数。
  - 无签名模式下，query 缓存复用条件从“非空文件”收紧为“行数与本次评测样本数一致”。
  - 日志区分为：
    - 命中：`检测到 query 缓存（无签名模式）`
    - 不命中：`query 缓存条数不匹配（无签名模式），重建`
- `scripts/memory_indexer/runtime/eval_hf_normal.py`
  - 默认 alias 从 `users` 改为 `users_normal`，避免覆盖/污染 followup 主缓存。

### 现场恢复
- 将 `users` 固定缓存恢复为 followup_plus_chat 对应 legacy v2 文件（不重算）：
  - `memory_cache_users.jsonl` 行数恢复为 `5449`
  - `eval_cache_users.jsonl` 行数恢复为 `4807`（含 1 行 meta + 4806 query）

## 2026-02-24（性能修复：缓存全命中时跳过 HF 初始化 + 计时落盘）
### 本次变更摘要
- `scripts/memory_indexer/eval_router.py`
  - 新增 cache-only fast path：当 memory/query 缓存均可直接复用时，跳过 `HFSentenceEncoder` / `E5TokenEncoder` 初始化。
  - 新增 `memory_cache_fully_reusable()`，按 `mem_id/text/encoder_id/strategy/coarse_role` 校验 memory 缓存是否完整命中。
  - 新增 `_CacheOnlyEncoder` 占位实现，并在误判时自动回退为真实编码器初始化，避免行为回归。
  - 增加评测计时输出：
    - `[timing] scoring_elapsed_s`
    - `[timing] total_elapsed_s`
  - 同步写入 `eval.metrics.json.timing` 字段。

- `scripts/memory_indexer/runtime/eval_followup_plus_chat_quick.py`
  - wrapper 总耗时写入 `eval.log.txt`：`[runtime] elapsed_s=...`，不再只打印到终端。

### 实测结果（quick, followup_plus_chat, 1000 queries）
- 命中日志出现：`[cache] query/memory 缓存完整命中，跳过 HF 编码器初始化。`
- 计时（`runs/rt/followup_plus_chat_pipeline/20260224_224413/pairwise/eval/eval.log.txt`）：
  - `scoring_elapsed_s=193.35`
  - `total_elapsed_s=231.55`
  - `runtime elapsed_s=232.35`

## 2026-02-25（评测并行提速：eval-workers + torch 线程控制）
### 本次变更摘要
- `scripts/memory_indexer/eval_router.py`
  - `evaluate_policy()` 新增 `eval_workers` 参数，支持 query 级线程并行（`ThreadPoolExecutor`）。
  - 每个 worker 独立构建 `Retriever/Router/Scorer`，避免共享状态引发一致性和并发问题。
  - `run_bootstrap()` 同步透传 `eval_workers`。
  - 新增 CLI 参数：
    - `--eval-workers`（默认读取配置，缺省 1）
    - `--torch-num-threads`（默认读取配置，缺省 0=不改）
  - 当设置 `--torch-num-threads > 0` 时，运行前会设置：
    - `torch.set_num_threads(n)`
    - `torch.set_num_interop_threads(1)`（若可设置）
  - `eval.metrics.json` 新增记录：
    - `eval_workers`
    - `torch_num_threads`
...
- `scripts/memory_indexer/runtime/eval_followup_plus_chat_quick.py`
  - 新增并设置默认参数：
    - `--eval-workers 4`
    - `--torch-num-threads 2`
  - 运行 quick 时默认按 4 worker 并行 + 每进程/线程池 2 线程策略执行。

### 实测记录（同配置：followup_plus_chat / top_n=30 / top_k=5 / half_hard / mix(auto)）
- 基线（单 worker，上一轮）：`scoring_elapsed_s=179.51`
  - `runs/rt/followup_plus_chat_pipeline/20260224_233740/pairwise/eval/eval.log.txt`
- 并行后（4 workers + torch threads=2）：`scoring_elapsed_s=165.65`
  - `runs/rt/followup_plus_chat_pipeline/20260225_000104/pairwise/eval/eval.log.txt`
- 结论：在当前 CPU/GPU 环境下，评分阶段得到可观提速；总耗时仍会受缓存重建与编码阶段影响。

### 补充复测（缓存全命中）
- `runs/rt/followup_plus_chat_pipeline/20260225_000634/pairwise/eval/eval.log.txt`
  - 命中日志：`[cache] query/memory 缓存完整命中，跳过 HF 编码器初始化。`
  - `scoring_elapsed_s=166.13`
  - `total_elapsed_s=204.72`

## 2026-02-26（P0/P1：coarse 矩阵化 + 细粒度计时）

- `src/memory_indexer/index.py`
  - `CoarseIndex.search` 增加 CPU 矩阵化路径（`numpy`）并缓存 memory 矩阵，避免逐条 Python `dot`。
  - top-n 选择改为 `argpartition + 局部排序`，避免每次全量排序。
  - 无 `numpy` 环境下回退为 `heapq.nlargest`，保持兼容。
- `src/memory_indexer/retriever.py`
  - 消除 lexical 重复搜索：候选构建与 lexical 分数共用一次 lexical 检索结果。
  - 新增 query 级分段计时并写入 `route_output.metrics`：
    - `coarse_elapsed_s`
    - `lexical_elapsed_s`
    - `scorer_elapsed_s`
    - `routing_elapsed_s`
    - `retrieve_elapsed_s`
- `scripts/memory_indexer/eval_router.py`
  - 聚合并输出上述分段计时（每 query 平均值）。
  - `eval.log` 新增行：
    - `timing/query_ms coarse=... | lexical=... | scorer=... | routing=... | total=...`
  - `eval.metrics.json` 的 `records[*].metrics` 同步包含分段计时均值与 total 字段。

## 2026-02-27（quick runtime 日志路径修复）

- `scripts/memory_indexer/runtime/eval_followup_plus_chat_quick.py`
  - 修复 wrapper 追加 `eval.log.txt` 时的路径问题：
    - 运行目录改为基于 `REPO_ROOT` 的绝对路径，避免在非仓库 cwd 下触发 `FileNotFoundError`。
    - 写日志前增加 `eval_log_path.parent.mkdir(parents=True, exist_ok=True)` 兜底。
    - 终端打印 `eval_log` 时优先显示相对仓库路径，便于阅读。
- 本地验证（不污染正式 users 缓存）：
  - `--cache-alias users_smoke --max-eval-queries 5`
  - wrapper 成功写入：
    - `runs/rt/followup_plus_chat_pipeline/20260227_000356/pairwise/eval/eval.log.txt`

## 2026-02-27（新增粗排基线指标：CoarseRankRecall@k）

- 目标：补齐“仅按粗召回排序时的 Recall@k”，用于直接对比 reranker 的净贡献。
- 指标命名：
  - 展示名：`CoarseRankRecall@k`
  - metrics 键：`coarse_rank_recall_at_k`
- `src/memory_indexer/retriever.py`
  - 在 `route_output.explain` 写入 `coarse_ranked_ids`（按 coarse_score 降序的候选 id 序列）。
- `scripts/memory_indexer/eval_router.py`
  - 聚合并输出 `coarse_rank_recall_at_k`。
  - 主输出新增：
    - `CoarseRankRecall@k=...`
    - `Gain@k=Recall@k - CoarseRankRecall@k`
  - `eval.metrics.json` 同步持久化该字段。
- 验证（users_smoke，5 queries）：
  - 控制台示例：`CoarseRankRecall@k=0.400 | Gain@k=0.000`
  - 产物示例：`runs/rt/followup_plus_chat_pipeline/20260227_010527/pairwise/eval/eval.metrics.json`

## 2026-02-27（P0/P1 收口：训练缓存短路 + Recall@P 对齐 + Listwise 权重更新）

### 本次变更摘要
- `scripts/memory_indexer/train_reranker.py`
  - 新增训练侧 cache-only 快路：当 query/memory 缓存完整可复用时，跳过 HF/E5 初始化。
  - query 缓存支持固定 alias + 无签名复用（与评测侧一致）。
  - 负例策略调整为“粗召回候选内非正例优先，候选不足再全库随机补齐”。
  - listwise 新增硬约束近似损失（默认开启）：
    - `--listwise-hard-constraint/--no-listwise-hard-constraint`
    - `--listwise-hard-weight`
- `scripts/memory_indexer/eval_router.py`
  - 新增并输出 `CoarseRankRecall@P`（`coarse_rank_recall_at_pos_count`）。
  - 核对并保持 `Recall@P = |Top_|P| ∩ P| / |P|` 口径（Top_|P| 取 reranker 排序）。
- `scripts/memory_indexer/runtime/eval_followup_plus_chat_quick.py`
  - 默认 reranker 改为 `data/ModelWeights/listwise_reranker.pt`。
  - 保持正式 alias 默认 `users`，不再混入 smoke 专用 alias。

### 运行与产物
- listwise 训练（30 epochs）：
  - `runs/rt/p2_listwise_retrain_20260227/train.log.txt`
  - `runs/rt/p2_listwise_retrain_20260227/train.metrics.json`
  - 覆盖输出：`data/ModelWeights/listwise_reranker.pt`
  - 覆盖前备份：`data/ModelWeights/listwise_reranker.old_20260227_211031.pt`
- quick 评测（listwise）：
  - `runs/rt/followup_plus_chat_pipeline/20260227_212209/listwise/eval/eval.log.txt`
  - `runs/rt/followup_plus_chat_pipeline/20260227_212209/listwise/eval/eval.metrics.json`
  - 指标示例：`Recall@k=0.367`，`Recall@P=0.349`，`CoarseRankRecall@k=0.402`，`CoarseRankRecall@P=0.387`，`Gain@k=-0.035`

### 说明
- 已确认 pairwise/listwise 未混用：
  - quick 默认评测路径当前指向 listwise；
  - `runs/rt/p1_pairwise_retrain_20260227/train.log.txt` 对应 pairwise 权重来源；
  - 当前 listwise 正式权重来源为 `runs/rt/p2_listwise_retrain_20260227/`。
