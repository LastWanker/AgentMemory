        ...
        if max(1, int(workers)) == 1:
            for idx, row in enumerate(memory_rows, start=1):
                ...
                if idx % 20 == 0 or idx == len(memory_rows):
                    print(f"[v4][slots] scanned={idx}/{len(memory_rows)} newly_written={processed}")
        else:
            with concurrent.futures.ThreadPoolExecutor(max_workers=max(1, int(workers))) as executor:
                ...
                for idx, future in enumerate(concurrent.futures.as_completed(future_map), start=1):
                    ...
                    if idx % 20 == 0 or idx == len(pending_rows):
                        print(f"[v4][slots] completed={idx}/{len(pending_rows)} newly_written={processed}")
        for item in slot_rows:
            f.write(json.dumps(item, ensure_ascii=False) + "\n")
        f.flush()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build V4 slots via DeepSeek IE.")
    ...
    parser.add_argument("--out", default="data/V4/processed/memory_slots.jsonl")
    ...
    print(f"[v4] slots count={result['count']} written_now={result['written_now']} -> {result['out']}")
