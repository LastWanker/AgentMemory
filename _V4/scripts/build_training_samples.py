...
def build_training_samples(config_path: str | Path, query_in: Path, out_path: Path, *, max_queries: int = 0) -> dict:
    ...
    query_slots_path = resolve_path(cfg_get(cfg, "query.slots_path", "data/V4/processed/query_slots.jsonl"))
    ...
    try:
        query_coarse_map, query_slot_vector_map = load_cache_maps(cache_dir, cache_alias, kind="query")
    except Exception:
        query_coarse_map, query_slot_vector_map = {}, {}
    ...
    for idx, row in enumerate(query_rows, start=1):
        ...
        cached_query_vec = query_coarse_map.get(query_id)
        pack = retriever.collect_candidate_pack_with_vector(query_text, query_vec=cached_query_vec)
        query_context = build_query_context(
            retriever.artifacts.dense_index,
            query_text,
            slot_row=query_slot_map.get(query_id),
            q_vec=cached_query_vec,
            query_slot_vectors=query_slot_vector_map.get(query_id),
        )
        ...
        if idx % 100 == 0:
            print(f"[v4][samples] {idx}/{len(query_rows)}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build V4 training samples.")
    ...
    parser.add_argument("--out", default="data/V4/training/train_samples.jsonl")
    ...
    print(
        f"[v4] training samples count={result['sample_count']} avg_list_len={result['avg_list_len']:.2f} "
        f"-> {result['out']}"
    )
