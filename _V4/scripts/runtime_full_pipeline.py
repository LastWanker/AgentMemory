...
def main() -> None:
    parser = argparse.ArgumentParser(description="V4 full runtime pipeline.")
    ...
    if args.mode in ("prepare", "all"):
        ...
        build_hybrid_index(root_dir / "processed" / "memory.jsonl", slots_path, root_dir, config_path=args.config)
        ...
        print(f"[v4][runtime] prepare done -> {root_dir}")
    if args.mode in ("train", "all"):
        train_model(
            TrainConfig(
                sample_path=root_dir / "training" / "train_samples.jsonl",
                model_path=resolve_path(cfg_get(cfg, "reranker.model_path", "data/V4/models/reranker.pt")),
                run_dir=resolve_path(Path("_V4") / "runs" / "reranker_train" / "runtime_full"),
        ...
        print("[v4][runtime] train done")
    if args.mode in ("eval", "all"):
        result = evaluate_offline(
            EvalConfig(
                config_path=args.config,
                query_path=root_dir / "processed" / "query.jsonl",
                run_dir=resolve_path(Path("_V4") / "runs" / "eval_offline" / "runtime_full"),
                split="test",
        ...
        print(
            f"[v4][runtime] eval done queries={result['query_count']} "
