...
def _resolve_device(requested: str, cfg: dict) -> tuple[str, int]:
    ...
    if preferred == "cuda" and not cuda_ok:
        print("[v4][train] cuda requested but unavailable, fallback to cpu")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train V4 reranker.")
    parser.add_argument("--config", default="_V4/configs/default.yaml")
    parser.add_argument("--samples", default="data/V4/training/train_samples.jsonl")
    ...
    model_out = args.model_out or cfg_get(
        ...
        "data/V4/models/slot_bipartite_reranker.pt" if family.startswith("slot_bipartite") else "data/V4/models/reranker.pt",
    )
    train_cfg = TrainConfig(
        sample_path=resolve_path(args.samples),
        model_path=resolve_path(model_out),
        run_dir=resolve_path(Path("_V4") / "runs" / "reranker_train" / timestamp),
        ...
        family=family,
        data_root=resolve_path(cfg_get(cfg, "data.root_dir", "data/V4")),
        query_slots_path=resolve_path(cfg_get(cfg, "query.slots_path", "data/V4/processed/query_slots.jsonl")),
        query_path=resolve_path("data/V4/processed/query.jsonl"),
    ...
    print(
        f"[v4] reranker family={family} device={resolved_device} batch_size={train_cfg.batch_size} "
