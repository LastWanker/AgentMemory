...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate V4 offline.")
    ...
    eval_cfg = EvalConfig(
        ...
        run_dir=resolve_path(Path("_V4") / "runs" / "eval_offline" / timestamp),
        split=args.split,
    ...
    print(f"[v4] eval split={args.split} queries={result['query_count']} -> {eval_cfg.run_dir}")
    print(
        f"[v4] coarse RetrievalRecall@N={result['coarse']['retrieval_recall_at_n']:.4f} "
