...
def main() -> None:
    parser = argparse.ArgumentParser(description="Export V4 chat bundle.")
    ...
    parser.add_argument("--out", default="data/V4/exports/chat_memory_bundle.jsonl")
    args = parser.parse_args()
    result = export_chat_bundle(resolve_path(args.memory_in), resolve_path(args.slots_in), resolve_path(args.out))
    print(f"[v4] chat bundle count={result['count']} -> {result['out']}")
