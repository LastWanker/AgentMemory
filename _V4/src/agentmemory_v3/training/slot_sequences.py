...
def load_query_slot_sequences(
    query_slots_path: Path,
    dense_index,
    *,
    query_path: Path | None = None,
    query_slot_vector_map: dict[str, dict[str, np.ndarray]] | None = None,
    ...
    for row_idx, query_id in enumerate(query_ids):
        cached_slot_vectors = (query_slot_vector_map or {}).get(query_id)
        if cached_slot_vectors:
            seq = np.stack(
                [np.asarray(cached_slot_vectors.get(field), dtype=np.float32) for field in SLOT_FIELDS],
                axis=0,
            ).astype(np.float32)
        else:
            seq = dense_index.transform_texts(
                [
                    " ".join(str(item) for item in row.get(field, []) if str(item).strip())
