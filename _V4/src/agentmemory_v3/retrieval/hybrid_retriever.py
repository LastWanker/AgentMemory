...
class HybridRetriever:
    ...
    def from_config(cls, config_path: str | Path) -> "HybridRetriever":
        ...
        feature_model_path = resolve_path(cfg_get(cfg, "reranker.fallback_model_path", "data/V4/models/reranker.pt"))
        retriever.feature_reranker_bundle = load_reranker(feature_model_path)
        if retriever.reranker_family.startswith("slot_bipartite"):
            model_path = resolve_path(cfg_get(cfg, "reranker.model_path", "data/V4/models/slot_bipartite_reranker.pt"))
    ...
    def retrieve_debug(
        ...
        *,
        ...
        trace["coarse_only"] = bool(coarse_only)
        trace["reranker_enabled"] = not bool(coarse_only)
        return {
    ...
    def rerank_candidates(
        ...
        *,
        ...
        query_context = build_query_context(
            self.artifacts.dense_index,
            query,
            slot_row=query_slot_row,
            q_vec=query_vec,
            query_slot_vectors=query_slot_vectors,
        )
        feature_rows = []
