...
def evaluate_offline(config: EvalConfig) -> dict:
    ...
    try:
        _query_coarse_map, _query_slot_vector_map = load_cache_maps(cache_dir, cache_alias, kind="query")
    except Exception:
        _query_coarse_map, _query_slot_vector_map = {}, {}
    ...
    for row in query_rows:
        ...
        debug = retriever.retrieve_debug(
            ...
            query_vec=_query_coarse_map.get(str(row.get("query_id") or "")),
            query_slot_vectors=_query_slot_vector_map.get(str(row.get("query_id") or "")),
        )
        ...
        if len(per_query_rows) % 100 == 0:
            print(f"[v4][eval] {len(per_query_rows)} queries")
